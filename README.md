...
> [!WARNING]
> Captail `v0.8.6` is an early public preview. Recording works, but bugs and hardware-specific problems are expected. NVIDIA GeForce GTX 1650 and RTX 40/50 series are tested; other GPUs need broader public testing.

## What's new in Captail 0.8.6?

Captail 0.8.6 corrects the interface-wide icon refresh:

- **Cleaner, lighter icons:** Captail's action and status icons now use a cohesive Lucide-based visual language with consistent rounded strokes, optical weight, and alignment at compact Windows display sizes. Brand marks and third-party application icons remain recognizable.
